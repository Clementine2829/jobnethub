...
class JobListing extends Component {
  ...
  jobsCounter = () => {
    ...
    return `${startIndex + 1} - ${startIndex + displayedJobs.length} of ${
  ...
  resetValues = () => {
    if (this.state.jobs.length > this.state.jobsPerPage) {
      ...
      if (remainder === 0) {
        this.state.pagesCounter =
          this.state.jobs.length / this.state.jobsPerPage;
      } else {
          ...
          <div className={`col-sm-10`}>
            <div className={`${container.browseJobs}`}>
              <div className={`${container.browseJobs}`}>
                ...
                <button className={`${container.btnFilter}`}>
                  Most recent
                </button>
                <button className={`${container.btnFilter}`}>
                  Most Viewed
