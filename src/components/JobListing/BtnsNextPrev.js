...
function CreateIndexedBtns({ jobs, activePage, pagesCounter, jobsPerPage, nextOrPrevPage }){
    const resultText = []
    ...
    if (jobs > jobsPerPage) {
        ...
        resultText.push(
            <button 
                key={0} 
                className={`${btns.btnsList} ${btns.active}`}>
                1
            </button>)
        prevCondition = true;
        nextCondition = true;
    ...
    return(
        <div className={`${btns.pageBtns}`}>
            <button 
                ...
                key={-1} className={`${btns.btn} ${prevCondition ? btns.disabled : ""}`}
            ...
            <button 
                onClick={() => nextOrPrevPage(activePage + 1)}
                key={-2} className={`${btns.btn} ${nextCondition ? btns.disabled : ""}`}
